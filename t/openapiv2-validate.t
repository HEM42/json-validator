...
my $schema = JSON::Validator::Schema::OpenAPIv2->new->data(
  $cwd->child(qw(spec v2-petstore.json)));
my $c = t::Helper->controller(\%params);

note 'dummy data from http request';
$params{get_req} = [{in => 'body', name => 'body', exists => 0},];
...
cmp_deeply(
  ...
  [
    superhashof({
      in => 'body', name => 'body', accepts => {'application/json' => ignore},
    }),
  ],
